...
st.markdown("""
    ...
    .tip-style {
        ...
        background-color: #f8fafc; border: 2px solid #023047;
        min-height: 140px; display: flex; align-items: center; justify-content: center;
        text-align: center; color: #023047; font-weight: 600; font-style: italic;
    }
    </style>
...
with st.sidebar:
    ...
    with st.expander("🚀 QUICK START GUIDE", expanded=True):
        ...
        <div class="guide-content">
            ...
            <b>3. API Token:</b> Admin Center > Apps > Zendesk API > Token Access ON.
        </div>
        """, unsafe_allow_html=True)
...
# --- 4. THE "UN-CORNY" TIPS ---
tips = [
    "💀 **Admin Truth:** If you don't fix these 404s, your customers will find them. And they will mention it in the CSAT comment.",
    "⚠️ **SEO Reality:** Google doesn't care how 'helpful' your article is if the first link 404s. It's an instant bounce.",
    "🥃 **Guerilla Tip:** Stop using 'New' in article titles. It stays 'New' for three years and makes you look like a liar.",
    "🛠️ **Workflow:** 1,800 articles? You don't have a Knowledge Base, you have a digital museum. Time to archive some relics.",
    "🛑 **Accessibility:** If your link text is 'Click here,' you're failing customers with screen readers. Do better.",
    "📉 **Stats:** 70% of 'I can't find the answer' tickets are actually 'I found the answer but the link was broken.'",
    "🕵️ **Deep Cut:** Zendesk search index is only as good as your meta-tags. If the article is trash, the search is trash.",
    "⚡ **Speed:** Manual audits are for people with too much time. Let the machine find the rot while you drink your coffee."
]
...
with tab1:
    if st.button("🚀 RUN DEEP SCAN"):
        if not all([subdomain, email, token]):
            ...
            with st.status("📡 Scraping Zendesk API...", expanded=True) as status:
                while api_url:
                    res = requests.get(api_url, auth=auth)
                    if res.status_code != 200: break
                    data = res.json()
                    all_articles.extend(data.get('articles', []))
                    st.write(f"📥 Pulled {len(all_articles)} headers...")
            ...
            if all_articles:
                ...
                with col_tip:
                    st.markdown("### 🗣️ Admin Reality Check")
                ...
                for i, art in enumerate(all_articles):
                    ...
                    if b_int or b_ext or typos:
                        ...
                        status_line = f"<span class='log-err'>[ROT]</span> {art['title'][:40]}..."
                    else:
                        status_line = f"<span class='log-msg'>[CLEAN]</span> {art['title'][:40]}..."
                    ...
                    stat_int.metric("Internal 404", total_int)
                    stat_ext.metric("External 404", total_ext)
                    stat_typo.metric("Typos", total_typo)

                st.success(f"Audit Complete. Found {len(report_list)} articles with rot.")
                st.balloons()